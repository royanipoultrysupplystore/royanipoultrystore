...
import { ArrowLeft, Phone, Store, Edit2, Plus, Trash2 } from 'lucide-react'
...
export default function MarketSellerDetail() {
  ...
  const totalReceived = transactions.reduce((s, tx) => s + (tx.total_amount || 0), 0)
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-green-50 rounded-xl p-4 border border-green-100 text-center">
          ...
          <p className="text-xs font-medium text-green-500 mt-1">{t('market.totalReceived')}</p>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50">
                ...
                <th className="text-end px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">{t('market.totalAmountPaid')}</th>
            ...
            <tfoot>
              <tr className="border-t-2 border-slate-200 bg-slate-50">
                ...
                <td className="px-4 py-3 text-end font-bold text-green-700">{formatCurrency(totalReceived)}</td>
                <td />
              </tr>
            </tfoot>
          </table>
            ...
            <div className="relative" ref={farmDropRef}>
              ...
              {txForm.farm_id && !farmDropOpen && (
                <button type="button" onClick={() => { setTxForm(f => ({ ...f, farm_id: '' })); setFarmSearch(''); setFarmDropOpen(true) }}
                  className="absolute end-2 top-1/2 -translate-y-1/2 text-xs text-slate-400 hover:text-slate-600">✕</button>
              )}
              {farmDropOpen && (
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input required type="number" min="1" value={txForm.chicken_count}
                onChange={e => setTxForm(f => ({ ...f, chicken_count: e.target.value }))}
                className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#2E86AB]/30" />
            </div>
            <div>
              ...
              <input type="number" min="0" step="0.01" value={txForm.price_per_chicken}
                onChange={e => setTxForm(f => ({ ...f, price_per_chicken: e.target.value }))}
                className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#2E86AB]/30" />
            </div>
          </div>

          <div>
            <label className="block text-xs font-medium text-slate-600 mb-1">{t('market.totalAmountPaid')} (AFN)</label>
            ...
            <p className="text-xs text-slate-400 mt-1">
              {t('dispatches.paidAtDispatch')} — {t('dispatches.willAddDebt').toLowerCase()} {t('farmDetail.payments').toLowerCase()}
            </p>
          </div>
